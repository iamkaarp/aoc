async function taskA(input: string): Promise<number> {
  let me = new Player({ hp: 50, mana: 500, turn: 0 }, true)
  let boss = new Player({ hp: 51, damageAmt: 9, turn: 0 }, false)
  let cheapestSpent = Infinity

  function playAllGames(me: Player, boss: Player) {
...
class Player {
  ...
  constructor(initial: Record<string, number>, isWizard: boolean) {
    this.history = []
    ...
    if (this.isWizard) {
      this.spells = [
        ...
        {
          cost: 113,
          start: (m: any, o: any) => (m.armor += 7),
          effect: (m: any, o: any) => {},
          end: (m: any, o: any) => (m.armor -= 7),
          duration: 6,
        },
        {
          cost: 173,
          effect: (m: any, o: any) => o.damage(3),
          duration: 6,
        },
        {
          cost: 229,
          effect: (m: any, o: any) => (m.mana += 101),
          duration: 5,
        },
      ]
    }
  ...
  attack(opponent: any, spellIdx: any = null) {
    if (!this.isWizard) {
